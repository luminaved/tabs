...
/**
 * Счётчики автора: сколько у него разборов, сколько из них подтверждено
 * модератором и сколько всего просмотров. Питают полосу на публичном профиле
 * (`/u/[id]`) и в личном кабинете.
 *
 * Это подмножество админской сводки, и намеренно маленькое. Наружу вынесено
 * ровно то, что и так видно, пролистав страницу автора руками: сами разборы,
 * галочки на них и счётчик просмотров с каждой страницы разбора. Всё
 * остальное — аккаунты, лайки, избранное, разбивка по видимости, ряды по
 * дням — остаётся за `getAdminStats`: это про то, как сайт живёт изнутри.
 */
...
/**
 * `publicOnly` — это граница приватности, а не удобство вызова.
 *
 * На ЧУЖОМ профиле считаем строго публичные разборы: иначе счётчик выдавал бы
 * посторонним, сколько у человека приватных и «по ссылке», — а это ровно то,
 * что он спрятал. Разница между «12 разборов» в полосе и десятью строками в
 * списке под ней и есть утечка, даже если самих песен не видно.
 *
 * В СВОЁМ кабинете считаем всё. Там это своя библиотека целиком, и число обязано
 * сойтись с бейджами «Гитара / Укулеле» рядом — те тоже считают разборы любой
 * видимости (см. countOwnByInstrument). Поэтому в кабинете число будет больше,
 * чем на собственном публичном профиле, и это не расхождение, а разные вопросы:
 * «сколько я написал» против «сколько из этого видно людям».
 */
export async function getUserStats(userId: string, publicOnly: boolean): Promise<UserStats> {
  const where = publicOnly ? { userId, visibility: 'public' } : { userId };

  const [totals, verified] = await Promise.all([
    // aggregate одним запросом отдаёт и счёт, и сумму просмотров: считать их
    // порознь — два похода в базу за тем, что берётся одним проходом.
    prisma.song.aggregate({ where, _count: { _all: true }, _sum: { viewCount: true } }),
    prisma.song.count({ where: { ...where, verified: true } }),
  ]);

  return {
    songs: totals._count._all,
    verified,
    // _sum по пустой выборке возвращает null, а не ноль.
