 ...
 *
 * Дорого это не только в хранении. `body` тянет за собой ещё две колонки:
 ...
 *   — из него же собирается `chords` для чипов в списках.
 *
 * Обе пересобираются при сохранении (см. normalize в lib/songs.ts) — то есть
 * ровно один раз на правку. Раньше третьим пунктом здесь стояло чтение `body`
 * списками: `cardSelect`/`catalogSelect` выбирали текст ради чипов, и страница
 * каталога тащила из базы двадцать четыре песни разом. Теперь не выбирают.
 *
