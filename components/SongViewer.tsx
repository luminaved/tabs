...
export function SongViewer({
   ...
   * остаётся на прежних 30px (формула в .song-title). Считается по СТРОКАМ, а
   * не по словам: сколько места нужно названию в одну строку и сколько в две.
   * Потолок для двухстрочных — 42px: две строки такого кегля вместе с
   * исполнителем ещё не выше обложки, то есть шапка не растёт.
   */
  const titleStyle = useMemo(() => {
    const fit = titleFit(titleText, optimisticVerified);
    return {
      '--title-w1': fit.oneLine,
      '--title-w2': fit.twoLines,
