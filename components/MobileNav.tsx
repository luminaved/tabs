 ...
 *
 * На экранах редактора её нет вовсе — см. `EDITOR_ROUTE`.
 */

/**
 * Экраны, где панели быть не должно: создание и правка разбора.
 *
 * Причина не в том, что она мешает эстетически. На телефоне там снизу уже стоит
 * панель, а сверху — шапка сайта; стоит тапнуть в текст, выезжает клавиатура, и
 * от самого текста песни остаётся щель в несколько строк. Навигация в другие
 * разделы посреди незаконченной правки не нужна: уход с этой страницы — это
 * «Отмена», и она на виду в шапке редактора.
 *
 * Регулярное выражение, а не список: id в адресе приходит с подписью
 * («/songs/kishlak-skuchno-abc123/edit», см. lib/slug.ts), точным перечнем такое
 * не покрыть.
 */
const EDITOR_ROUTE = /^\/songs\/new$|^\/songs\/[^/]+\/edit$/;
export function MobileNav({
  ...
  // Выход ПОСЛЕ хуков, а не до: порядок вызовов хуков обязан совпадать между
  // рендерами, а переход на редактор и обратно происходит без размонтирования.
  ...
  return (
    ...
    >
      <div className="mnav-inner">
        {/* Каталогов два — гитарный («/») и укулельный («/ukulele»), — а пункт
            один: на телефоне пятая кнопка не помещается, а переключатель
            инструментов и так стоит первым на самой странице каталога. Но
            подсвечивать пункт надо на ОБОИХ: пока условие было `pathname === '/'`,
            на укулельном каталоге не подсвечивалось вообще ничего, и панель
            выглядела так, будто человек находится вне сайта. */}
        <NavItem
          href="/"
          label="Каталог"
          active={pathname === '/' || pathname === '/ukulele'}
        >
          <SearchIcon />
        </NavItem>
