 ...
 *
 * Заведено после того, как внешний вход не появлялся на боевом сервере, а
 * понять почему было нельзя: ненастроенный провайдер снаружи выглядит ровно
 * так же, как настроенный неправильно, — пустым местом. Каждая догадка стоила
 * деплоя.
 *
 * Показываются ТОЛЬКО факты наличия, никогда сами значения: секрет провайдера —
 * это доступ к входу. Страница админская, но секретам всё равно незачем
 * появляться в разметке.
 */
